...
namespace DigitBridge.CommerceCentral.ERP.Integration.Api
{
    ...
    public static class EventProcessBroker
    {
        ...
        [FunctionName(nameof(UpdateSyncResult))]
        [OpenApiOperation(operationId: "UpdateSyncResult", tags: new[] { "Eventprocesss" })]
        [OpenApiParameter(name: "masterAccountNum", In = ParameterLocation.Header, Required = true, Type = typeof(int), Summary = "MasterAccountNum", Description = "From login profile", Visibility = OpenApiVisibilityType.Advanced)]
        [OpenApiParameter(name: "profileNum", In = ParameterLocation.Header, Required = true, Type = typeof(int), Summary = "ProfileNum", Description = "From login profile", Visibility = OpenApiVisibilityType.Advanced)]
        [OpenApiParameter(name: "code", In = ParameterLocation.Query, Required = true, Type = typeof(string), Summary = "API Keys", Description = "Azure Function App key", Visibility = OpenApiVisibilityType.Advanced)]
        [OpenApiRequestBody(contentType: "application/json", bodyType: typeof(SyncResultPayload), Description = "UpdateEventDto ")]
        [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "application/json", bodyType: typeof(EventProcessResponsePayload))]
        public static async Task<JsonNetResponse<EventProcessResponsePayload>> UpdateSyncResult(
            [HttpTrigger(AuthorizationLevel.Function, "PATCH", Route = "eventProcesss/UpdateSyncResult")]
            HttpRequest req)
        {
            var rquestData = await req.GetBodyObjectAsync<SyncResultPayload>();
            var payload = await req.GetParameters<EventProcessERPPayload>();
            var dbFactory = await MyAppHelper.CreateDefaultDatabaseAsync(payload);
            var svc = new EventProcessERPService(dbFactory);
            var success = await svc.UpdateActionStatusAsync(rquestData);
