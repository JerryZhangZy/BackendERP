...
namespace DigitBridge.QuickBooks.Integration
{
    ...
    /// </summary>
    [Serializable()]
    public partial class QuickBooksSettingInfoDataDtoCsv : CsvHelper<QuickBooksSettingInfoDataDto>
    {
        ...
        public override void ReadEntities(CsvReader csv, IList<QuickBooksSettingInfoDataDto> data)
        {
            var isFirst = true;
            QuickBooksSettingInfoDataDto dto = new QuickBooksSettingInfoDataDto();
            while (csv.Read())
            {
                // it is header line
                ...
                switch (csv.GetField(0))
                {
                    case "H":
                        if (!isFirst)
                        {
                            if (dto != null && dto.HasQuickBooksChnlAccSetting)
                                data.Add(dto);
                            dto = new QuickBooksSettingInfoDataDto();
                            isFirst = false;
                        }
                        dto. QuickBooksChnlAccSetting = csv.GetRecord<QuickBooksChnlAccSettingDto>();
                        break;
                    //case "L":
