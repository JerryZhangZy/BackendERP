...
namespace DigitBridge.CommerceCentral.YoPoco
{
    ...
    public class StructureTable
    {
        ...
        public string AlliesName { get; set; }
        ...
        public string LoadByColumnType
        {
            get
            {
                var col = ParentTable.GetColumn(LoadByColumn);
                return (col == null) ? "string" : col.PropertyType;
            }
        }
    ...
    public class SqlTableInfo
    {
        ...
        public List<SqlColumnInfo> JsonColumns { get; set; }
        public SqlColumnInfo UpdateDateUtcColumn { get; set; }
        public string Text { get; set; }

        public bool HasJsonColumns { get { return JsonColumns != null && JsonColumns.Count > 0; } }

        public bool HasUpdateDateUtcColumn { get { return UpdateDateUtcColumn != null; } }
        ...
        {
            return Columns.FirstOrDefault(c => c.Name.Equals(name, StringComparison.CurrentCultureIgnoreCase));
        }
        ...
        public string NonRowNumPrimaryKey
        {
            get
            {
                return string.IsNullOrEmpty(PrimaryKey) || PrimaryKey.Equals("RowNum", StringComparison.CurrentCultureIgnoreCase)
                    ? null
                    : $"_{PrimaryKey.First().ToString().ToLower()}{PrimaryKey.Substring(1)}";
            }
        }
    ...
    public class SqlColumnInfo
    {
        ...
        public string jsonDictionaryName
        {
            get { return $"{Name.Substring(4)}"; }
        }

        public bool isJson
        {
            get { return isString && Name.StartsWith("Json", StringComparison.CurrentCultureIgnoreCase); }
        }
        ...
        public string GetFakeRule()
        {
            ...
            if (isJson)
                return $".RuleFor(u => u.{jsonDictionaryName}, (f, u) => u.{jsonDictionaryName}.SetValues(f.Random.JObject()))";
                ...
                case "string":
                    gen = (IsUniqueKey || Name.ToLower().EndsWith("id"))
                        ...
                        : (Length == "50")
                            ...
                            : (Length.Equals("max", StringComparison.CurrentCultureIgnoreCase))
                                ? $"f => f.Lorem.Sentence()"
    ...
    public class SQLFileParser
    {
        ...
        public StructureInfo GetStructure()
        {
            ...
            foreach (var stu in Structure.StructureTables)
            {
                ...
                stuTb.Table.AlliesName = stuTb.AlliesName;
        ...
        protected SqlTableInfo ParseSQLFile(string content)
        {
            ...
            table.JsonColumns = table.Columns.Where(x => x.isJson).ToList();
            table.UpdateDateUtcColumn = table.Columns.Where(i => i.Name.ToLower() == "updatedateutc").FirstOrDefault();
        ...
        {
            var columnRegx = new Regex(@"\[(\S+)\]\s+\[?(\w+)\]?(?:\((\d+|max|\d+,\s?\d+)\))?(?:\s+(?:[\(\w\,'\(\)]+)){1,10},(\s*--([^\[]+))?", RegexOptions.IgnoreCase | RegexOptions.Multiline);
            ...
            try
            {
                ...
                if (matchCols.Count > 0)
                {
                    foreach (Match m in matchCols)
                    {
                        ...
                        col.IsPrimaryKey = !string.IsNullOrEmpty(table.PrimaryKey) && table.PrimaryKey.Equals(col.Name, StringComparison.CurrentCultureIgnoreCase);
        ...
        protected SqlColumnInfo GetPropertyAtrribute(SqlColumnInfo col)
        {
            ...
            if (col.isJson)
                col.PropertyAtrributes.Add($"[XmlIgnore, JsonIgnore, IgnoreCompare]");
