...
{
    /// <summary>
    /// Represents a CustomerAddressDataDtoCsv Class.
    /// NOTE: This class is generated from a T4 template Once - you you wanr re-generate it, you need delete cs file and generate again
    /// </summary>
    [Serializable()]
    public partial class CustomerAddressDataDtoCsv : CsvHelper<CustomerAddressDataDto>
    {
        ...
        protected override void WriteCsv(CustomerAddressDataDto data, CsvWriter csv)
        {
        ...
        public override void ReadEntities(CsvReader csv, IList<CustomerAddressDataDto> data)
        {
            var isFirst = true;
            CustomerAddressDataDto dto = new CustomerAddressDataDto();
            while (csv.Read())
            {
                // it is header line
                ...
                switch (csv.GetField(0))
                {
                    case "H":
                        if (!isFirst)
                        {
                            if (dto != null && dto.HasCustomerAddress)
                                data.Add(dto);
                            dto = new CustomerAddressDataDto();
                            isFirst = false;
                        }
                        dto. CustomerAddress = csv.GetRecord<CustomerAddressDto>();
                        break;
                    //case "L":
