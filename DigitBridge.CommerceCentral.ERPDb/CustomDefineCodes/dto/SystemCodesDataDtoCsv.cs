...
{
    /// <summary>
    /// Represents a SystemCodesDataDtoCsv Class.
    /// NOTE: This class is generated from a T4 template Once - you you wanr re-generate it, you need delete cs file and generate again
    /// </summary>
    [Serializable()]
    public partial class SystemCodesDataDtoCsv : CsvHelper<SystemCodesDataDto>
    {
        ...
        {
			context.RegisterClassMap(new CsvAutoMapper<SystemCodesDto>());
        }
        
        protected override void WriteCsv(SystemCodesDataDto data, CsvWriter csv)
        {
        ...
        public override void ReadEntities(CsvReader csv, IList<SystemCodesDataDto> data)
        {
            var isFirst = true;
            SystemCodesDataDto dto = new SystemCodesDataDto();
            while (csv.Read())
            {
                // it is header line
                ...
                switch (csv.GetField(0))
                {
                    case "H":
                        if (!isFirst)
                        {
                            if (dto != null && dto.HasSystemCodes)
                                data.Add(dto);
                            dto = new SystemCodesDataDto();
                            isFirst = false;
                        }
                        dto. SystemCodes = csv.GetRecord<SystemCodesDto>();
                        break;
                    //case "L":
