...
{
    /// <summary>
    /// Represents a InitNumbersDataDtoCsv Class.
    /// NOTE: This class is generated from a T4 template Once - you you wanr re-generate it, you need delete cs file and generate again
    /// </summary>
    [Serializable()]
    public partial class InitNumbersDataDtoCsv : CsvHelper<InitNumbersDataDto>
    {
        ...
        {
			context.RegisterClassMap(new CsvAutoMapper<InitNumbersDto>());
        }
        
        protected override void WriteCsv(InitNumbersDataDto data, CsvWriter csv)
        {
        ...
        public override void ReadEntities(CsvReader csv, IList<InitNumbersDataDto> data)
        {
            var isFirst = true;
            InitNumbersDataDto dto = new InitNumbersDataDto();
            while (csv.Read())
            {
                // it is header line
                ...
                switch (csv.GetField(0))
                {
                    case "H":
                        if (!isFirst)
                        {
                            if (dto != null && dto.HasInitNumbers)
                                data.Add(dto);
                            dto = new InitNumbersDataDto();
                            isFirst = false;
                        }
                        dto. InitNumbers = csv.GetRecord<InitNumbersDto>();
                        break;
                    //case "L":
