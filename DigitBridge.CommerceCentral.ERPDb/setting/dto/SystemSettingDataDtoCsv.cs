...
{
    /// <summary>
    /// Represents a SystemSettingDataDtoCsv Class.
    /// NOTE: This class is generated from a T4 template Once - you you wanr re-generate it, you need delete cs file and generate again
    /// </summary>
    [Serializable()]
    public partial class SystemSettingDataDtoCsv : CsvHelper<SystemSettingDataDto>
    {
        ...
        {
			context.RegisterClassMap(new CsvAutoMapper<SystemSettingDto>());
        }
        
        protected override void WriteCsv(SystemSettingDataDto data, CsvWriter csv)
        {
        ...
        public override void ReadEntities(CsvReader csv, IList<SystemSettingDataDto> data)
        {
            var isFirst = true;
            SystemSettingDataDto dto = new SystemSettingDataDto();
            while (csv.Read())
            {
                // it is header line
                ...
                switch (csv.GetField(0))
                {
                    case "H":
                        if (!isFirst)
                        {
                            if (dto != null && dto.HasSystemSetting)
                                data.Add(dto);
                            dto = new SystemSettingDataDto();
                            isFirst = false;
                        }
                        dto. SystemSetting = csv.GetRecord<SystemSettingDto>();
                        break;
                    //case "L":
