...
{
    /// <summary>
    /// Represents a ActivityLogDataDtoCsv Class.
    /// NOTE: This class is generated from a T4 template Once - you you wanr re-generate it, you need delete cs file and generate again
    /// </summary>
    [Serializable()]
    public partial class ActivityLogDataDtoCsv : CsvHelper<ActivityLogDataDto>
    {
        ...
        {
			context.RegisterClassMap(new CsvAutoMapper<ActivityLogDto>());
        }
        
        protected override void WriteCsv(ActivityLogDataDto data, CsvWriter csv)
        {
        ...
        public override void ReadEntities(CsvReader csv, IList<ActivityLogDataDto> data)
        {
            var isFirst = true;
            ActivityLogDataDto dto = new ActivityLogDataDto();
            while (csv.Read())
            {
                // it is header line
                ...
                switch (csv.GetField(0))
                {
                    case "H":
                        if (!isFirst)
                        {
                            if (dto != null && dto.HasActivityLog)
                                data.Add(dto);
                            dto = new ActivityLogDataDto();
                            isFirst = false;
                        }
                        dto. ActivityLog = csv.GetRecord<ActivityLogDto>();
                        break;
                    //case "L":
