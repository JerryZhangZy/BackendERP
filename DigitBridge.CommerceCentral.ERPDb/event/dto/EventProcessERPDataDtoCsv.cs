...
{
    /// <summary>
    /// Represents a EventProcessERPDataDtoCsv Class.
    /// NOTE: This class is generated from a T4 template Once - you you wanr re-generate it, you need delete cs file and generate again
    /// </summary>
    [Serializable()]
    public partial class EventProcessERPDataDtoCsv : CsvHelper<EventProcessERPDataDto>
    {
        ...
        {
			context.RegisterClassMap(new CsvAutoMapper<EventProcessERPDto>());
        }
        
        protected override void WriteCsv(EventProcessERPDataDto data, CsvWriter csv)
        {
        ...
        public override void ReadEntities(CsvReader csv, IList<EventProcessERPDataDto> data)
        {
            var isFirst = true;
            EventProcessERPDataDto dto = new EventProcessERPDataDto();
            while (csv.Read())
            {
                // it is header line
                ...
                switch (csv.GetField(0))
                {
                    case "H":
                        if (!isFirst)
                        {
                            if (dto != null && dto.HasEventProcessERP)
                                data.Add(dto);
                            dto = new EventProcessERPDataDto();
                            isFirst = false;
                        }
                        dto. EventProcessERP = csv.GetRecord<EventProcessERPDto>();
                        break;
                    //case "L":
