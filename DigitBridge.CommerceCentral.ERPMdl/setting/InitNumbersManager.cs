...
namespace DigitBridge.CommerceCentral.ERPMdl
{
    ...
    public class InitNumbersManager :  IInitNumbersManager , IMessage
    {
        ...
        public async Task<byte[]> ExportAsync(InitNumbersPayload payload)
        {
            var rowNumList =await initNumbersList.GetRowNumListAsync(payload);
            var dtoList = new List<InitNumbersDataDto>();
           foreach(var x in rowNumList)
            {
                if (initNumbersService.GetData(x))
                    dtoList.Add(initNumbersService.ToDto());
            };
            if (dtoList.Count == 0)
                dtoList.Add(new InitNumbersDataDto());
            return initNumbersDataDtoCsv.Export(dtoList);
        }

        public byte[] Export(InitNumbersPayload payload)
        {
            var rowNumList =initNumbersList.GetRowNumList(payload);
            var dtoList = new List<InitNumbersDataDto>();
            foreach (var x in rowNumList)
            {
                if (initNumbersService.GetData(x))
        ...
        public void Import(InitNumbersPayload payload, IFormFileCollection files)
        {
            ...
            foreach(var file in files)
            {
                ...
                var list = initNumbersDataDtoCsv.Import(file.OpenReadStream());
                var readcount = list.Count();
                var addsucccount = 0;
                var errorcount = 0;
                foreach(var item in list)
                {
                    payload.InitNumbers = item;
                    if (initNumbersService.Add(payload))
                        addsucccount++;
                    else
                    {
                        errorcount++;
                        foreach (var msg in initNumbersService.Messages)
                            Messages.Add(msg);
                        initNumbersService.Messages.Clear();
                    }
                }
                if (payload.HasInitNumbers)
                    payload.InitNumbers = null;
                AddInfo($"File:{file.FileName},Read {readcount},Import Succ {addsucccount},Import Fail {errorcount}.");
            }
        }

        public async Task ImportAsync(InitNumbersPayload payload, IFormFileCollection files)
        {
            if(files==null||files.Count==0)
            {
                AddError("no files upload");
            ...
            foreach(var file in files)
            {
                ...
                {
                    payload.InitNumbers = item;
                    if (await initNumbersService.AddAsync(payload))
                        addsucccount++;
                    else
                    {
                        errorcount++;
