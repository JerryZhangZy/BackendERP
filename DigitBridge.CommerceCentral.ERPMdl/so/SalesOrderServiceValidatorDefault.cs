    ...
    public partial class SalesOrderServiceValidatorDefault : IValidator<SalesOrderData, SalesOrderDataDto>, IMessage
    {
        ...
        public virtual async Task<bool> ValidateAccountAsync(IPayload payload, string number = null, ProcessingMode processingMode = ProcessingMode.Edit)
        {
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(number))
                {
                    using (var tx = new ScopedTransaction(dbFactory))
                    {
                        if (!await SalesOrderHelper.ExistNumberAsync(number, pl.MasterAccountNum, pl.ProfileNum).ConfigureAwait(false))
                        {
                            isValid = false;
                            AddError($"Data not found.");
                        }
                ...
                else
                {
                    using (var tx = new ScopedTransaction(dbFactory))
                    {
                        if (!await SalesOrderHelper.ExistRowNumAsync(dto.SalesOrderHeader.RowNum.ToLong(), pl.MasterAccountNum, pl.ProfileNum).ConfigureAwait(false))
                        {
                            isValid = false;
                            AddError($"Data not found.");
                        }
        ...
        public virtual async Task<bool> ValidateAsync(SalesOrderDataDto dto, ProcessingMode processingMode = ProcessingMode.Edit)
        {
            ...
            if (processingMode == ProcessingMode.Add)
            {
                ...
                //UK check OrderNumber&profilenum exist
                if (!string.IsNullOrEmpty(dto.SalesOrderHeader.OrderNumber)&&
                    await SalesOrderHelper.ExistNumberAsync(dto.SalesOrderHeader.OrderNumber, dto.SalesOrderHeader.ProfileNum.ToInt()).ConfigureAwait(false))
                {
                    isValid = false;
                    AddError($"[OrderNumber:{dto.SalesOrderHeader.OrderNumber},ProfileNum:{dto.SalesOrderHeader.ProfileNum}] exist");
                }
            }
            if (processingMode == ProcessingMode.Edit)
            {
