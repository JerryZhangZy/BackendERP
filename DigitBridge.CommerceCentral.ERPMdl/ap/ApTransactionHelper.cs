...
{
    /// <summary>
    /// Represents a ApTransactionHelper SQL Helper Static Class.
    /// NOTE: This class is generated from a T4 template Once - you you wanr re-generate it, you need delete cs file and generate again
    /// </summary>
    public static class ApTransactionHelper
    {
        public static bool ExistNumber(string number, int masterAccountNum, int profileNum)
        {
/*
            var sql = $@"
SELECT COUNT(1) FROM ApInvoiceTransaction tbl
...
";
        ...
        {
            var sql = $@"
SELECT RowNum FROM ApInvoiceTransaction tbl
WHERE MasterAccountNum = @masterAccountNum
AND ProfileNum = @profileNum";
...
*/
        ...
        public static async Task<int> GetTranSeqNumAsync(string apInvoiceNum, int profileNum)
        {
            var sql = $@"
SELECT isnull(max(TransNum),0)+1 FROM ApInvoiceTransaction tbl
WHERE ProfileNum = @profileNum
AND ApInvoiceNum= @apInvoiceNum
";
            var result = await SqlQuery.ExecuteScalarAsync<int>(sql,
                profileNum.ToSqlParameter("profileNum"),
                apInvoiceNum.ToSqlParameter("apInvoiceNum")
            );
            return result;
        }
        public static int GetTranSeqNum(string apInvoiceNum, int profileNum)
        {
            var sql = $@"
SELECT isnull(max(TransNum),0)+1 FROM ApInvoiceTransaction tbl
WHERE ProfileNum = @profileNum
