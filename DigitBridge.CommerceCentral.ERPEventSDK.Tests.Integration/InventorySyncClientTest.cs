...
namespace DigitBridge.CommerceCentral.ERPEventSDK.Tests.Integration
{
    ...
    public partial class InventorySyncClientTest : IDisposable, IClassFixture<TestFixture<StartupTest>>
    {
        ...
        {
            var client = new InventorySyncClient(_baseUrl, _code);
            var data = new InventorySyncUpdateDataDto
            {
                MasterAccountNum = 10001,
                ProfileNum = 10001,
                 InventorySyncItems=new List<InventorySyncItemsDto>() { new InventorySyncItemsDto() { SKU= "Bike", WarehouseCode= "nobis", Qty=20 } }
